...
chrome.runtime.onMessage.addListener(function(request, sender) {
  ...
  <div id="MemeMachineChromeExtensionModal">
    ...
    <div class="content">
      <button type="button" onclick="this.parentElement.parentElement.outerHTML = '';">
  ...
  function updateText(){
    ...
    context.lineWidth = 2;
    context.fillText(document.querySelector("div#MemeMachineChromeExtensionModal div.content div.row div.inputs input:first-child").value, canvas.width / 2, topOffset + remToPx(3));
    context.strokeText(document.querySelector("div#MemeMachineChromeExtensionModal div.content div.row div.inputs input:first-child").value, canvas.width / 2, topOffset + remToPx(3));
    context.fillText(document.querySelector("div#MemeMachineChromeExtensionModal div.content div.row div.inputs input:last-child").value, canvas.width / 2, canvas.height - topOffset - remToPx(1));
    context.strokeText(document.querySelector("div#MemeMachineChromeExtensionModal div.content div.row div.inputs input:last-child").value, canvas.width / 2, canvas.height - topOffset - remToPx(1));
  }
  ...
  document.querySelector("div#MemeMachineChromeExtensionModal div.content div.row > button").onclick = function(){
    chrome.runtime.sendMessage({imageURL: imageURL, width: canvas.width, height: canvas.height, topText: document.querySelector("div#MemeMachineChromeExtensionModal div.content div.row div.inputs input:first-child").value, bottomText: document.querySelector("div#MemeMachineChromeExtensionModal div.content div.row div.inputs input:last-child").value});
  }
});
